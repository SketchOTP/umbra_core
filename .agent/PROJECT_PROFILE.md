Current status: UMBRA-AS-003J ACTIVE from `c736db0594e21abfcd5d472d5af3b0cdd7d3780c` after accepted AS-003I `AS003I_CROSS_OWNER_SALIENCE_CALIBRATION_PRIMITIVE_REQUIRED`. AS-003J is a zero-run/non-production ontology and calibration replan: criteria lock, source/retained-artifact audit, causal-coverage proof, and static projection only. It tests whether temporal, habit, development, memory, relationship context, and opportunity are causal modulators rather than peer motivational owners, while preserving all required behavioral influence. It cannot implement an adapter, add weights/normalization/global utility/source priority, run an organism, or start AS-003K/AS-004/CLOSE-03.
