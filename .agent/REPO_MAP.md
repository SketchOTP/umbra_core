...
- D-010 Stage B freeze (tip `5d218d6`, invalidated `6943981`): `experiments/d010/freeze_stage_b.py`, `formal-execution-contract.json` (`d010-fe-stage-b-v2`); harness `_organism_cfg` pins `OrganismConfig.condition` to C0 with ablations via `temporal_config`; `test_harness_integrated_ablations_create_organism_and_tick`; frozen preregistration bundle; `run_seal.py` contract-only cross-check
