## UMBRA-AS-003G active — simultaneous-context control convergence gate — 2026-08-31

`UMBRA-AS-003G` is active from exact local/GitHub `master` `016b71417c3420fdf119aa253e029a0a013ae9d3`. `AS003F_SIMULTANEOUS_CONTEXT_RESOLUTION_PRIMITIVE_REQUIRED` is accepted. This is non-production, zero-organism-run architecture research that must separately decide no-incumbent initial election, incumbent persistence, and switching/starvation for simultaneous non-hard contexts. It may use complete AS-003F evidence, sealed AS-003C corpus, current static source, qualified fixtures, and bounded primary prior art; it must lock candidate control semantics before any offline projection.

No production/test change, organism run, diagnostic rerun, retry, reseed, V1 revival, global score, cross-system weight, source priority, fixed owner hierarchy, arbitrary timeout/hazard/incumbent coefficient, scheduler fairness, planner/RL/active-inference/POMDP/MPC, AS-003H, AS-004, or CLOSE-03 is authorized. Richman et al. is reference-only: persistent goal bouts and stochastic transitions are relevant qualitatively, but its calibrated relative-need, gradient, and noise parameters are prohibited imports. Evidence root: `/srv/ATLAS/100_ACTIVE/Projects/UMBRA-CORE/evidence/live-evidence/umbra-as-003g-simultaneous-context-control-r1/`.
