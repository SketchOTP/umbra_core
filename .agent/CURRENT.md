## UMBRA-AS-003J governance start — owner ontology / regulatory calibration — 2026-08-31

`AS003I_CROSS_OWNER_SALIENCE_CALIBRATION_PRIMITIVE_REQUIRED` is accepted. `UMBRA-AS-003J` begins from exact local/GitHub `master` `c736db0594e21abfcd5d472d5af3b0cdd7d3780c` as zero-run, non-production architecture research. It will lock role criteria before reclassifying physiology, temporal expectation, habit, development, memory, relationship/social state, opportunity, individuality, commitment, and learned consequence associations. The hypothesis is that only true regulatory drives need a common control calibration; incentives, associations, habits, continuity, memory, temporal context, and developmental capability must retain causal participation without being promoted to peer motivational owners. No production/test change, organism execution, diagnostic rerun, retry/reseed, V1 revival, generic normalization, owner weights, source priority, global utility/reward/affection, planner/RL/active-inference/POMDP/MPC, AS-003K, AS-004, or CLOSE-03 is authorized. Evidence root: `/srv/ATLAS/100_ACTIVE/Projects/UMBRA-CORE/evidence/live-evidence/umbra-as-003j-owner-ontology-calibration-r1/`.
