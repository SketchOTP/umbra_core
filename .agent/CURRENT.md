## Current authority — UMBRA-AS-003C replay migration and fresh qualification — 2026-08-30

`UMBRA-AS-003B_RETAINED_REPLAY_FAIL` is accepted terminal. `UMBRA-AS-003C` starts from exact master `327f747612626e3894192879ce961489e9308d6a` as a new bounded retained-replay migration and distributed-competition qualification generation, not a retry. The frozen AS-003B `INSPECT` expected / `ORIENT` observed nonpass is Architect-classified `RETAINED_REPLAY_MIGRATION_DEFECT`: it monkeypatched ordinary legacy scalar `score_candidate` totals, which AS-001/AS-002 superseded. The still-qualified property is that no admissible optional intent can replace the ordinary canonical base-pool authority path. Before a new dedicated freeze, AS-003C must audit every retained replay assertion, migrate only exclusively superseded scalar assertions, preserve all other qualified properties, and run only source-proven pure tests/static validators. Production source must remain unchanged from AS-003B freeze `5c2642ac9c1c0be6340d583caf594f5799ecda13` unless a separately proven AS-002 contract defect exists. No organism, broad pytest, known R1, viability population, AS-004, CLOSE-03, retry, reseed, hard-coded winner, weights, source priority, planner, or post-freeze remediation is authorized. Evidence: `/srv/ATLAS/100_ACTIVE/Projects/UMBRA-CORE/evidence/live-evidence/umbra-as-003c-replay-migration-qualification-r1/`; packet: `.agent/tasks/active/UMBRA-AS-003C/`.
