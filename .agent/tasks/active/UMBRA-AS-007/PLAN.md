...
1. Reconcile the exact baseline, permanent AS-006 evidence, Notion authority,
   and zero-run counters; publish a governance-start record.
2. Attribute the retained R1 terminal trace and audit S16 recovery-contract
   coherence before changing production.
3. If coherent, lock and implement the smallest shared terminal-executability
   contract, including initial critical-recovery gating and current/remembered
   opportunity handling.
4. Run focused proofs, observer/development gates, and applicable regressions
   twice before freezing the scientific sequence.
5. Execute the frozen A/B/R1 sequence exactly once; stop permanently at the
   first authorized scientific failure.
