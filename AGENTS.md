...
- Digital Cell, protocell, and digital chemistry work are external to UMBRA CORE; **do not** create or execute UMBRA-D-000A

**Program gate:** UMBRA-D-000 closed via D-000S (`UMBRA_D000S_FOUNDATION_ARCHITECTURE_QUALIFIED`). **UMBRA-D-001 closed** — `UMBRA_D001_INVARIANT_COMPANION_CORE_QUALIFIED`. **UMBRA-D-002 closed** — `UMBRA_D002_SENSORIMOTOR_SELF_MODEL_QUALIFIED`. **UMBRA-D-002V** — `UMBRA_D002V_PERFORMANCE_FAIL` (preserved). **UMBRA-D-002P** — `UMBRA_D002P_PERFORMANCE_REMEDIATION_QUALIFIED`. **UMBRA-D-003 closed** — `UMBRA_D003_PREDICTIVE_WORLD_MODEL_QUALIFIED`. **UMBRA-D-004 closed** — `UMBRA_D004_INTRINSIC_DEVELOPMENT_QUALIFIED`. **UMBRA-D-005 closed** — `UMBRA_D005_MEMORY_CONSOLIDATION_QUALIFIED`. **UMBRA-D-006 closed** — `UMBRA_D006_SOCIAL_CONTINGENCY_QUALIFIED`. **UMBRA-D-007 closed** — `UMBRA_D007_LIVED_INDIVIDUALITY_QUALIFIED`. **UMBRA-D-008 closed** — `UMBRA_D008_COHERENT_DIGITAL_EMBODIMENT_QUALIFIED` (Supplement S3). **Qualified release baseline: D-009** — `UMBRA_D009_PERSISTENT_HABITAT_AGENCY_QUALIFIED`, seal `af35371`. **D-010 verdict: `UMBRA_D010_PERFORMANCE_FAIL`** — Gates 0–12 PASS; Gate 13 FAIL; Stage B v7 NOT CREATED; parent Mimir `9adf61b087ea4fa6a90a1c3bd401a9b3` OPEN. Next scientific work is not authorized by UMBRA-G-001.
