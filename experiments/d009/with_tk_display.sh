#!/usr/bin/env bash
# Environment for D-009 Task 14 Tkinter + Xvfb when system python3-tk is absent.
# Uses a user-local dpkg extract under ~/.local/umbratk (see D-008 Task 14 setup).
set -euo pipefail
...
if [[ -z "${DISPLAY:-}" ]]; then
  if ! xdpyinfo -display :99 >/dev/null 2>&1; then
    Xvfb :99 -screen 0 1280x720x24 >/tmp/umbra-d009-xvfb99.log 2>&1 &
    echo $! >/tmp/umbra-d009-xvfb99.pid
    sleep 1
  fi
  export DISPLAY=:99
fi
