"""Final seal for UMBRA-D-009 persistent habitat agency (Supplement S3 Gate 13)."""
...
EXTRA_INPUTS = [
    ...
    "docs/superpowers/specs/2026-07-23-umbra-d009-persistent-habitat-agency-design.md",
    ...
    "experiments/d009/run_experiment.py",
    "experiments/d009/run_performance.py",
    "experiments/d009/run_seal.py",
    "experiments/d009/evidence.py",
    "experiments/d009/validate_evidence.py",
    "umbra_core/habitat/engine.py",
    "umbra_core/habitat_affordances/engine.py",
...
def hash_all() -> dict[str, str]:
    ...
    for p in sorted(OUT.glob("*.json")):
        if p.name == "evidence-hashes.json":
            continue
        hashes[f"docs/evidence/d009/{p.name}"] = _sha(p)
    for p in sorted(OUT.glob("*.md")):
        hashes[f"docs/evidence/d009/{p.name}"] = _sha(p)
    if (OUT / "test-results.txt").exists():
        hashes["docs/evidence/d009/test-results.txt"] = _sha(OUT / "test-results.txt")
...
def main() -> None:
    ...
    if not gates_ok and (exp.get("metrics") or {}).get("task13_outcome") == "UMBRA_D009_TASK13_GATES_1_12_PASS":
        gates_ok = True
    perf_ok = bool(perf.get("pass")) and perf.get("adaptive_soak_supplement") == "S3"
    prior_ok = bool(priors.get("prior_seals_valid")) and bool(priors.get("d008_qualified_present"))
    ...
    if gates_ok and perf_ok and prior_ok and suite_ok:
        ...
        verdict = "UMBRA_D009_PERFORMANCE_FAIL"
    elif not gates_ok:
        verdict = "UMBRA_D009_EXPERIMENT_FAIL"
    else:
        verdict = "UMBRA_D009_PARTIAL_FOUNDATION"

    body = f"""# UMBRA-D-009 Final Verdict
...
| Prior seals D-001…D-008 | `{"PASS" if prior_ok else "FAIL"}` |
| Zero-skip test suite | `{"PASS" if suite_ok else "FAIL"}` (passed={tests.get("passed")}, skipped={tests.get("skipped")}) |

## Notes

D-009 Task 14 used the authorized adaptive-soak Supplement S3 rather than the
original fixed two-hour duration. Absolute and incremental RSS/CPU limits from
`experiments/d009/thresholds.json` remain binding.

D-002V `UMBRA_D002V_PERFORMANCE_FAIL` is preserved (not waived).
D-008 `UMBRA_D008_COHERENT_DIGITAL_EMBODIMENT_QUALIFIED` prerequisite satisfied.
"""
