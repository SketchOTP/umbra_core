"""AS-012 exact-entrypoint boundedness, soak, and matched ablations.
...
def boundedness(seed: int, work: Path, ticks: int = 100_000) -> dict[str, Any]:
    ...
    for index in range(ticks):
        ...
        if index == 0 or (index + 1) % 5000 == 0:
            row = {"tick": index + 1, "elapsed_seconds": time.perf_counter() - started, "rss_mib": float(current_rss_mib()), "db_bytes": db_bytes(db), **_counts(organism)}
    ...
    result: dict[str, Any] = {
        "schema": "AS012_BOUNDEDNESS_RESULT_V1", "directive": DIRECTIVE, "baseline": BASELINE, "seed": seed,
...
def _run_window(organism: Any, seconds: float, interval: float, journal: Path, samples: list[dict[str, float]]) -> tuple[int, float]:
    ...
    try:
        while time.monotonic() < end:
            ...
            if elapsed >= next_sample:
                row = {"elapsed_seconds": elapsed, "tick": organism.tick, "rss_mib": float(current_rss_mib()), **_counts(organism)}
...
def soak(seed: int, work: Path, *, warmup_seconds: float = SOAK["warmup_seconds"], measure_seconds: float = SOAK["measure_seconds"]) -> dict[str, Any]:
    ...
    warmup_ticks, _ = _run_window(organism, warmup_seconds, SOAK["sample_interval_seconds"], journal, warmup_samples)
    measure_ticks, measure_elapsed = _run_window(organism, measure_seconds, SOAK["sample_interval_seconds"], journal, measure_samples)
    final = _finalize_and_restart(organism, engine, seed, db, "R0")
    ...
    result: dict[str, Any] = {
        "schema": "AS012_REALTIME_SOAK_RESULT_V1", "directive": DIRECTIVE, "baseline": BASELINE, "seed": seed,
...
def ablation(seed: int, work: Path, variant: str, ticks: int = 7200) -> dict[str, Any]:
    ...
    result = {
        "schema": "AS012_ABLATION_RESULT_V1", "directive": DIRECTIVE, "baseline": BASELINE,
...
def main() -> None:
    ...
    with tmp.open("xb", encoding="utf-8") as handle:
