"""Task 13 evidence envelope helpers — schema, hashing, stats, preflight."""
...
EXP = ROOT / "experiments" / "d008"
OUT = ROOT / "docs" / "evidence" / "d008"

DIRECTIVE = "UMBRA-D-008"
AGENT_MEMORY = "D-20260723-umbra-d008-coherent-digital-embodiment"
...
def load_frozen() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, str]]:
    thr = json.loads((EXP / "thresholds.json").read_text())
    matrix = json.loads((EXP / "experiment-matrix.json").read_text())
    scen = json.loads((EXP / "scenario-suite.json").read_text())
    hashes = {
...
def preflight(
    ...
    *,
    ...
    if paired_seeds < min_seeds and allow_smoke:
        # Smoke may run, but dump() still refuses pass:true below 100.
        pass
    for hid, hval in thr.get("production_profile_definition_hashes", {}).items():
        if "PLACEHOLDER" in str(hval):
            raise SystemExit(f"preflight_fail:unresolved_placeholder:{hid}")
    # Re-hash and refuse mismatch against what we just loaded (integrity of freeze).
    live = {
        "thresholds_hash": file_sha256(EXP / "thresholds.json"),
        "matrix_hash": file_sha256(EXP / "experiment-matrix.json"),
        "scenario_suite_hash": file_sha256(EXP / "scenario-suite.json"),
    ...
    if live != hashes:
        raise SystemExit(f"preflight_fail:frozen_hash_drift:{live}")
...
def paired_ci(deltas: list[float], confidence: float = 0.95) -> list[float]:
    """Normal approx CI for paired mean delta (sufficient for large n>=100)."""
    n = len(deltas)
    if n < 2:
        return [0.0, 0.0]
    m = mean(deltas)
...
def comparison(
    *,
    ...
    """Paired comparison; pass requires length match and threshold on A (and gap if set)."""
...
def envelope(
    *,
    ...
    # Never allow pass:true when structural integrity fails
...
def dump(name: str, payload: dict[str, Any]) -> None:
    ...
    # Refuse to write pass:true without required schema fields
