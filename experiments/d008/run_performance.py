"""UMBRA-D-008 Task 14 performance — Supplement S3 adaptive soak.

Modes (fresh process recommended via --mode):
  P0  Core only (condition=C10, expression off)
  P1  Core + ExpressionEngine + HeadlessRenderer
  P2  Core + ExpressionEngine + TkinterRenderer (real Canvas + event loop)

Also: accelerated 100k boundedness, renderer lifecycle stress (≥100 cycles).

Timing from experiments/d008/performance-protocol.json (S3). Absolute RSS/CPU
limits remain in thresholds.json. Fixed two-hour soak is NOT run.
"""
...
def _smoke_scale() -> float:
    """D008_PERF_SMOKE=1 shortens walls for structural dry-runs (not sealable)."""
    if os.environ.get("D008_PERF_SMOKE") == "1":
        return float(os.environ.get("D008_PERF_SMOKE_SCALE", "0.02"))
    return 1.0
...
def _base_cfg(db_path: str, *, mode: str, hz: float, seed: int) -> OrganismConfig:
    expression = mode != "P0"
    condition = "C10" if mode == "P0" else "C0"
...
def run_100k() -> dict[str, Any]:
    ...
    out = {
        ...
        "frame_ring_capacity_limit": int(THR["frame_ring_capacity"]),
    ...
    try:
        for i in range(cycles):
            ...
            try:
                ...
                pass  # destroyed root raises — expected
            org.tick_once()
...
def run_mode(mode: str) -> dict[str, Any]:
    ...
    # Approximate per-sample cpu fraction from cumulative cpu_s.
    cpu_fracs = []
    ...
    out = {
        ...
        "profile_hashes": {
            ABSTRACT_SHAPE_BODY.profile_id: profile_definition_hash(ABSTRACT_SHAPE_BODY),
        },
...
def recompose(
    ...
    expression_over_core = _delta(p1, p0)
    ...
    # Expression incremental vs core, and tk vs headless / core, use ui_incremental limits.
    inc_pass = inc_ok(expression_over_core) and inc_ok(tkinter_over_headless) and inc_ok(tkinter_over_core)
    smoke = any(m.get("smoke_scaled") for m in (p0, p1, p2))
    out = {
        "directive": "UMBRA-D-008",
        "adaptive_soak_supplement": "S3",
        "supplement_note": (
            "D-008 Task 14 used authorized adaptive-soak Supplement S3 "
        ...
        "expression_over_core": expression_over_core,
    ...
    # Convenience soak summary for Gate 12 test readers (S3 duration, not 7200).
    out["soak"] = {
        "protocol": "S3",
...
def main() -> None:
    ...
    # all — sequential fresh-process modes to avoid shared-DB hazards
    results: dict[str, Any] = {}
    for step in ("100k", "lifecycle", "P0", "P1", "P2"):
        print(f"=== running {step} ===", flush=True)
        env = os.environ.copy()
        proc = subprocess.run(
            [sys.executable, "-m", "experiments.d008.run_performance", "--mode", step],
            cwd=ROOT,
            env=env,
        )
