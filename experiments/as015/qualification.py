"""AS-015-owned R0--R3 population execution.

The unchanged R0--R3 intervention semantics are reused from AS-014.  This
module owns the AS-015 namespace, result schema, and frozen population record;
it does not modify historical AS-014 artifacts.
"""
...
def execute(
    ...
    *,
    ...
    if (
        ...
        raise RuntimeError("AS015_FORMAL_MANIFEST_INVALID")
        ...
        for index, seed in enumerate(regimes[regime]):
            ...
            if row["terminal"] != "completed":
                return {
                    ...
                    "terminal": f"AS015_FRESH_{regime}_FAIL",
                    "rows": rows,
                }
    return {
        "schema": "AS015_FORMAL_POPULATION_V1",
