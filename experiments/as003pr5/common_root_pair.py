#!/usr/bin/env python3
"""One-shot common-root CONTROL/SHADOW protocol for UMBRA-AS-003P-R5.
...
from experiments.as003pr5.analysis import analyze as analyze_modal
from experiments.as003pr5.semantic_comparator import (
    collect_declared_ids,
    compare_run_records,
    compare_values,
)
from tools.as003pr5_evidence import ROOT as EVIDENCE_ROOT, canonical_json, publish
...
REGIME = "R0"
SCENARIO = "S0"
WORK_ROOT = EVIDENCE_ROOT / "r5-work"
LOCK_NAME = "AS003PR5_EXECUTION_PROTOCOL_LOCK.json"
...
def _snapshot_metadata(database: Path) -> dict[str, Any]:
    with sqlite3.connect(database) as connection:
        connection.row_factory = sqlite3.Row
        latest_row = connection.execute("SELECT value FROM meta WHERE key='latest_snapshot'").fetchone()
        ledger_row = connection.execute("SELECT value FROM meta WHERE key='ledger_tip'").fetchone()
        latest_id = json.loads(latest_row[0]) if latest_row else None
        snapshot = connection.execute(
            "SELECT snapshot_id, sequence, monotonic_time, state_hash FROM snapshots WHERE snapshot_id=?",
            (latest_id,),
        ).fetchone()
        return {
...
def _root_phase() -> dict[str, Any]:
    ...
    root_db = WORK_ROOT / "shared-root.sqlite"
    organism = None
    root_creation_started = time.time()
    try:
        organism, engine = fixture.prepare(SEED, root_db, REGIME)
        if organism.tick != 0:
            raise RuntimeError(f"root_measured_tick_violation:{organism.tick}")
        snapshot_id = organism.snapshot_if_due(force=True)
        organism.store.validate_chain()
        root_state = _safe(organism.authoritative_state())
        root_rng = _safe(organism.rng.export_state())
        root_events = _safe(list(organism.store.iter_events()))
        habitat_state = copy.deepcopy(engine.state)
        habitat_canonical = canonical_serialize(habitat_state)
        habitat_bytes = json.dumps(habitat_canonical, sort_keys=True, separators=(",", ":")).encode()
        habitat_pickle = pickle.dumps(habitat_state, protocol=5)
    finally:
        if organism is not None:
            organism.close()

    habitat_path = WORK_ROOT / "shared-habitat.pickle"
    _atomic_write(habitat_path, habitat_pickle)
    metadata = _snapshot_metadata(root_db)
    if metadata["latest_snapshot_id"] != snapshot_id:
        raise RuntimeError("forced_snapshot_not_latest")
    root_inventory = _database_inventory(root_db)
    control_db = WORK_ROOT / "control.sqlite"
    shadow_db = WORK_ROOT / "shadow.sqlite"
    _backup_database(root_db, control_db)
    _backup_database(root_db, shadow_db)
    ...
    independent_files = len({os.stat(path).st_ino for path in (root_db, control_db, shadow_db)}) == 3
    clone_proof = {
        "schema": "AS003PR5_ROOT_CLONE_PROOF_V1",
        "directive": "UMBRA-AS-003P-R5",
        "result": "PASS" if clone_equal and independent_files else "FAIL",
        "method": "quiescent SQLite Connection.backup after forced snapshot and root close",
        "root_database_sha256": _digest_file(root_db),
        ...
        "independent_inodes": independent_files,
        "shared_writable_files": False,
    }
    ...
    root_record = {
        "schema": "AS003PR5_SHARED_ROOT_RECORD_V1",
        "directive": "UMBRA-AS-003P-R5",
        "fixture": {"regime": REGIME, "scenario": SCENARIO, "seed": SEED},
        "creation_count": 1,
        "measured_ticks": 0,
        "root_creation_started_unix": root_creation_started,
        "root_creation_finished_unix": time.time(),
        "snapshot": metadata,
        "snapshot_returned_id": snapshot_id,
        "root_event_count": len(root_events),
        "root_authoritative_state_sha256": _digest_bytes(canonical_json(root_state)),
        "root_rng_sha256": _digest_bytes(canonical_json(root_rng)),
        "root_habitat_sha256": _digest_bytes(habitat_bytes),
        "root_habitat_pickle_sha256": _digest_bytes(habitat_pickle),
        "pre_fork_exact_ids": exact_ids,
        "pre_fork_exact_id_count": len(exact_ids),
    }
    _publish_json("AS003PR5_SHARED_ROOT_AUTHORITATIVE_STATE.json", root_state)
    _publish_json("AS003PR5_SHARED_ROOT_RNG.json", root_rng)
    _publish_json("AS003PR5_SHARED_ROOT_HABITAT.json", habitat_canonical)
    _publish_json("AS003PR5_SHARED_ROOT_EVENTS.json", root_events)
    _publish_json("AS003PR5_SHARED_ROOT_RECORD.json", root_record)
    _publish_json("AS003PR5_ROOT_CLONE_PROOF.json", clone_proof)
    return {
        "record": root_record,
...
def _branch(role: str, database: Path, habitat_path: Path, work: Path) -> int:
    ...
    try:
        ...
        organism.store.validate_chain()
        pre = {
            "schema": "AS003PR5_BRANCH_PREMEASUREMENT_V1",
            "role": role,
        ...
        record = {
            "schema": "AS003PR5_BRANCH_RAW_EXECUTION_V1",
            "role": role,
            "seed": SEED,
            "horizon": HORIZON,
            "shadow_enabled": shadow,
        ...
        failure = {
            "schema": "AS003PR5_BRANCH_EXCEPTION_V1",
            "role": role,
...
def _orchestrate() -> int:
    if (EVIDENCE_ROOT / "AS003PR5_PAIRED_EXECUTION_FINISHED.json").exists():
        raise FileExistsError("R5 one-shot protocol already completed")
    implementation_commit = _git("rev-parse", "HEAD")
    root = _root_phase()
    workers = {
        "CONTROL": _start_worker("CONTROL", root["control_db"], root["habitat_path"]),
        "SHADOW": _start_worker("SHADOW", root["shadow_db"], root["habitat_path"]),
    }
    try:
        ...
        pre_parity.update(
            {
                "schema": "AS003PR5_PREMEASUREMENT_PARITY_V1",
                "directive": "UMBRA-AS-003P-R5",
        ...
        _publish_json("AS003PR5_CONTROL_PREMEASUREMENT.json", control_pre)
        _publish_json("AS003PR5_SHADOW_PREMEASUREMENT.json", shadow_pre)
        _publish_json("AS003PR5_PREMEASUREMENT_PARITY.json", pre_parity)
        if not pre_parity["semantic_equal"]:
            _stop_workers(workers)
            _publish_json(
                "AS003PR5_SCIENTIFIC_RESULT.json",
                {
                    "schema": "AS003PR5_SCIENTIFIC_RESULT_V1",
                    "verdict": "AS003PR5_PREMEASUREMENT_PARITY_FAIL",
            ...
            return 0
        barrier = {
            "schema": "AS003PR5_PREMEASUREMENT_BARRIER_V1",
            "result": "READY_FOR_EXECUTION_LOCK",
            "implementation_commit": implementation_commit,
            "root_record_sha256": _digest_file(EVIDENCE_ROOT / "AS003PR5_SHARED_ROOT_RECORD.json"),
            "clone_proof_sha256": _digest_file(EVIDENCE_ROOT / "AS003PR5_ROOT_CLONE_PROOF.json"),
            "premeasurement_parity_sha256": _digest_file(EVIDENCE_ROOT / "AS003PR5_PREMEASUREMENT_PARITY.json"),
            "branch_process_ids": {role: worker.pid for role, worker in workers.items()},
            "measured_ticks": {"CONTROL": 0, "SHADOW": 0},
        }
        _publish_json("AS003PR5_PREMEASUREMENT_BARRIER.json", barrier)
        print("AS003PR5_BARRIER_READY", flush=True)
        ...
        forbidden = [
            ...
            if not (path.startswith(".agent/") or path == "experiments/as003pr5/AS003PR5_EXECUTION_PROTOCOL_LOCK.json")
        ]
        if forbidden:
        ...
        if any(row["exit_code"] != 0 for row in outputs.values()):
            for role in workers:
                ...
                if exception.exists():
                    _publish_file(f"AS003PR5_{role}_EXCEPTION.json", exception)
            _publish_json(
                "AS003PR5_FROZEN_EXECUTION_FAILURE.json",
                {"verdict": "AS003PR5_FROZEN_EXECUTION_FAIL", "worker_results": outputs, "no_retry": True},
            )
            return 0
        ...
        artifact_sha = {
            "AS003PR5_CONTROL_RUN_RAW.json": _publish_json("AS003PR5_CONTROL_RUN_RAW.json", control),
            "AS003PR5_SHADOW_RUN_RAW.json": _publish_json("AS003PR5_SHADOW_RUN_RAW.json", shadow),
            "AS003PR5_CONTROL_DECISION_TRACE.jsonl": _publish_file(
                "AS003PR5_CONTROL_DECISION_TRACE.jsonl", WORK_ROOT / "CONTROL.decision-trace.jsonl"
            ),
            "AS003PR5_SHADOW_DECISION_TRACE.jsonl": _publish_file(
                "AS003PR5_SHADOW_DECISION_TRACE.jsonl", WORK_ROOT / "SHADOW.decision-trace.jsonl"
            ),
            "AS003PR5_PLANNING_SHADOW_TRACE.jsonl": _publish_file(
                "AS003PR5_PLANNING_SHADOW_TRACE.jsonl", WORK_ROOT / "SHADOW.planning-shadow.jsonl"
            ),
        }
        ...
        artifact_sha["AS003PR5_SEMANTIC_OBSERVER_PARITY.json"] = _publish_json(
            "AS003PR5_SEMANTIC_OBSERVER_PARITY.json", parity
        )
        if not parity["semantic_equal"]:
            verdict = "AS003PR5_TRUE_OBSERVER_EFFECT_FAIL"
            blocker = None
            relation = None
        else:
            analyses = analyze_modal(
                EVIDENCE_ROOT / "AS003PR5_PLANNING_SHADOW_TRACE.jsonl",
                EVIDENCE_ROOT / "AS003PR5_SHADOW_DECISION_TRACE.jsonl",
            )
            for name, value in analyses.items():
                artifact_sha[name] = _publish_json(name, value)
            summary = analyses["AS003PR5_MODAL_EVIDENCE_SUMMARY.json"]
            blocker = analyses["AS003PR5_AS003L_BLOCKER_RESULT.json"]["classification"]
            relation = analyses["AS003PR5_AS002_FUTURE_BOUNDARY.json"]["disposition"]
            if summary["frames_complete"] == 0:
                verdict = "AS003PR5_SOURCE_FRAME_CAPTURE_FAIL"
            elif blocker == "BLOCKER_EXPRESSED":
                verdict = "AS003PR5_OBSERVER_SAFE_MODAL_EVIDENCE_QUALIFIED"
            else:
                verdict = "AS003PR5_OBSERVER_SAFE_MODAL_EVIDENCE_NONDISCRIMINATING"
        result = {
            "schema": "AS003PR5_SCIENTIFIC_RESULT_V1",
            "directive": "UMBRA-AS-003P-R5",
            "verdict": verdict,
        ...
        _publish_json("AS003PR5_SCIENTIFIC_RESULT.json", result)
        _publish_json(
            "AS003PR5_PAIRED_EXECUTION_FINISHED.json",
            {
                "schema": "AS003PR5_PAIRED_EXECUTION_FINISHED_V1",
        ...
        _stop_workers(workers)
        if not (EVIDENCE_ROOT / "AS003PR5_PROTOCOL_EXCEPTION.json").exists():
            _publish_json(
                "AS003PR5_PROTOCOL_EXCEPTION.json",
                {
                    "schema": "AS003PR5_PROTOCOL_EXCEPTION_V1",
                    "exception_type": type(error).__name__,
                    "exception": str(error),
                    "no_retry": True,
                },
