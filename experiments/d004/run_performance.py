"""D-004 performance: 100k accelerated ticks + RUNTIME_READY-anchored 2h soak."""
...
def run_100k(db: Path) -> dict:
    ...
    bounded = org.development.counts_bounded()
    org.close()
    org2 = load_organism(
        OrganismConfig(
            db_path=str(db),
    ...
    ok = org2.identity.agent_id == aid and len(org2.development.goals) == n_goals
    org2.close()
...
def run_soak(duration_s: float, sample_interval: float = 30.0) -> dict:
    ...
    samples: list[dict] = []
    next_sample = t0
    log_path = ROOT / "docs/evidence/d004" / "soak-2h.jsonl"
...
def main() -> None:
    ...
    mode = os.environ.get("UMBRA_D004_PERF_MODE", "all")

    result: dict = {}
    if mode in ("100k", "all"):
        work = ROOT / ".soak" / "d004_perf"
        work.mkdir(parents=True, exist_ok=True)
    ...
    if mode in ("soak", "all"):
        duration = float(os.environ.get("UMBRA_D004_SOAK_SECONDS", "7200"))
