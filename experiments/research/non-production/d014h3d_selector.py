"""D-014H3D fresh bounded prospective selector.
...
ORDINARY_FIELDS = (
    "regulatory_gain", "risk", "effort", "option_preservation",
    "novelty", "uncertainty_reduction", "habit_support", "development_readiness",
    "memory_support", "social_support", "world_model_confidence",
    "temporal_support", "individuality_modifier",
)
...
def _add_vector(left: dict[str, float], right: dict[str, float]) -> dict[str, float]:
    return {name: left[name] + right.get(name, 0.0) for name in DIMENSIONS}
...
def evaluate(state: dict[str, Any]) -> dict[str, Any]:
    required = {
        ...
        "effect_branches", "opportunities", "recovery_focus", "candidates",
    }
