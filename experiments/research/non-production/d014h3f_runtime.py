"""D-014H3F regime-faithful runtime adapter and bounded proof harness."""
...
def h3f_selector_callback(context: dict[str, Any]) -> dict[str, Any]:
    state = selector_input(context)
    result = evaluate(state)
    selected = result.get("selected")
    if not isinstance(selected, dict):
        raise RuntimeError("d014h3f_no_selected_candidate")
    return {
