...
from d014h3i_runtime import identity_selector_callback, run_one_tick, sentinel_selector_callback
...
def test_identity_hook_keeps_exact_action_path():
    ...
    assert hooked["trace"][0]["d014h3i_selector"]["post_selection_replacement_count"] == 0

def test_sentinel_is_constrained_to_captured_pool():
    result = run_one_tick(sentinel_selector_callback)
    selector_rows = [row for row in result["trace"] if "d014h3i_selector" in row]
    assert selector_rows
    row = selector_rows[0]
    selected = row["d014h3i_selector"]["selected_candidate"]
    pool = row["d014h3i_selector"]["candidate_pool"]
    assert any(
    ...
    assert row["d014h3i_selector"]["post_selection_replacement_count"] == 0
