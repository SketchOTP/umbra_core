#!/usr/bin/env python3
"""D-014H3E regime-faithful non-production runner."""
...
from d014h3e_runtime import HORIZON, KNOWN_R1, R0_SEEDS, h3e_selector_callback, identity_selector_callback, prepare_organism
...
DIRECTIVE = "UMBRA-D-014H3E"
BASELINE = "04fc267213aedb7f7e50185c9103a727075e2a8f"
EVIDENCE_ROOT = Path("/srv/ATLAS/100_ACTIVE/Projects/UMBRA-CORE/evidence/live-evidence/d014h3e-regime-faithful-integrated-selector-r1")
...
def adapter_burst(org: object, seed: int, tick: int) -> bool:
    ...
    envelope = SyntheticPerceptionAdapter(manifest).submit(observation_id=f"h3e-{seed}-{tick}",
        source_id=f"h3e-source-{seed % 4}", modality="body_telemetry", schema_version="v1",
        ...
        provenance_chain=({"step": "d014h3e"},), privacy_classification="DERIVED_ONLY",
...
def run_case(seed: int, regime: str, horizon: int, selector=h3e_selector_callback,
             capture_trace: bool = False) -> dict[str, object]:
    scenario = SCENARIOS[regime]
    work = Path("/dev/shm")
    db = work / f"h3e-{regime}-{seed}.sqlite"
    trace_path = work / f"h3e-{regime}-{seed}.jsonl"
    ...
    events = [build_initialized_event(state["engine"].state, event_id=f"h3e:{regime}:{seed}:init",
        transaction_id=f"h3e:{regime}:{seed}:init-tx", request_id=f"h3e:{regime}:{seed}:init-req")]
    ...
    try:
        for _ in range(horizon):
            ...
            if regime == "R2" and tick == 600:
                events.append(state["engine"].commit_object_creation(partner_object(), event_id=f"h3e:{seed}:create",
                    transaction_id=f"h3e:{seed}:create-tx", request_id=f"h3e:{seed}:create-req")); state["partner_created"] = True
            ...
            if regime == "R2" and tick == 1800:
                ...
                org = load_organism(__import__('d014h3e_runtime').organism_config(db, seed, selector, trace_path, scenario))
            ...
            if regime == "R2" and tick == 2400:
                events.append(state["engine"].commit_object_visibility("social:partner:d014", occluded=True, event_id=f"h3e:{seed}:occlude",
                    transaction_id=f"h3e:{seed}:occlude-tx", request_id=f"h3e:{seed}:occlude-req")); state["partner_occluded"] = True
            if regime == "R2" and tick == 2600:
                events.append(state["engine"].commit_object_visibility("social:partner:d014", occluded=False, event_id=f"h3e:{seed}:reappear",
                    transaction_id=f"h3e:{seed}:reappear-tx", request_id=f"h3e:{seed}:reappear-req")); state["partner_reappeared"] = True
...
def proof() -> dict[str, object]:
    ...
    if ordinary["actions"] != identity["actions"]: raise SystemExit("D014H3E_DISABLED_PARITY_FAIL")
    ...
    write_json(EVIDENCE_ROOT / "D014H3E_INJECTION_AND_REPLAY_PROOF.json", result); return result
...
def main() -> None:
    ...
    if args.mode == "proof": result = proof()
    elif args.mode == "preflight":
        result = {"r2": run_case(41241905, "R2", 2601), "r3": run_case(41241905, "R3", 3601)}
        write_json(args.evidence_root / "D014H3E_REGIME_PREFLIGHT.json", result)
        ...
        write_json(args.evidence_root / "D014H3E_REGIME_CONTRACT.json", contract); (args.evidence_root / "D014H3E_REGIME_CONTRACT.sha256").write_text(hashlib.sha256(canonical_bytes(contract)).hexdigest() + "\n"); result = contract
    elif args.mode == "r0":
        result = {"rows": [run_case(seed, "R0", args.horizon or HORIZON) for seed in R0_SEEDS]}
        result["all_pass"] = all(row["terminal"] == "completed" for row in result["rows"]); write_json(args.evidence_root / "D014H3E_R0_RESULTS.json", result)
    elif args.mode == "known-r1":
        result = run_case(KNOWN_R1, "R1", args.horizon or HORIZON); write_json(args.evidence_root / "D014H3E_KNOWN_R1_RESULT.json", result)
    else:
        result = {"holdouts": "adoption pending", "source_manifest": "D014H3D_FRESH_HOLDOUT_MANIFEST.json"}; write_json(args.evidence_root / "D014H3E_HOLDOUT_ADOPTION.json", result)
