"""D-014H3H deterministic authority-safe runtime adapter and proof harness."""
...
from d014h3h_selector import evaluate
...
def h3h_selector_callback(context: dict[str, Any]) -> dict[str, Any]:
    state = selector_input(context)
    result = evaluate(state)
    selected = result.get("selected")
    if not isinstance(selected, dict):
        raise RuntimeError("d014h3h_no_selected_candidate")
    return {
