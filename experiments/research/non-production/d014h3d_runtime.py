"""D-014H3D runtime injection adapter and bounded proof harness."""
...
def selector_input(context: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, row in enumerate(pool):
        ...
        policy_context = {
            "policy_visible": True,
            "evidence_refs": [f"runtime:{row.get('source_name', 'unknown')}:{index}"],
...
def organism_config(db_path: Path, seed: int, selector: Callable | None,
                    trace_path: Path) -> OrganismConfig:
...
def prepare_organism(db_path: Path, seed: int, selector: Callable | None,
                     trace_path: Path):
    from experiments.d009.run_experiment import _habitat_state_for_scenario
    config = organism_config(db_path, seed, selector, trace_path)
...
def run_one_tick(selector: Callable | None, seed: int = 41241905) -> dict[str, Any]:
...
def run_r0_case(seed: int, selector: Callable, horizon: int = HORIZON) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="u-h3d-", dir="/dev/shm") as tmp:
        root = Path(tmp)
        trace = root / "decision.jsonl"
        org = prepare_organism(root / "organism.sqlite", seed, selector, trace)
