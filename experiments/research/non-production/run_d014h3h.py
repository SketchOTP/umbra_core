#!/usr/bin/env python3
"""D-014H3H regime-faithful non-production runner."""
...
from d014h3h_runtime import HORIZON, KNOWN_R1, R0_SEEDS, h3h_selector_callback, identity_selector_callback, prepare_organism
from d014h3h_selector import canonical_bytes, fingerprint
from test_d014h3h_selector import base_state
...
BASELINE = "b2435089ccf11e3c3e354443115ee61934003aff"
EVIDENCE_ROOT = Path("/srv/ATLAS/100_ACTIVE/Projects/UMBRA-CORE/evidence/live-evidence/d014h3h-deterministic-authority-safe-selector-r1")
...
def adapter_burst(org: object, seed: int, tick: int) -> bool:
    ...
    envelope = SyntheticPerceptionAdapter(manifest).submit(observation_id=f"h3h-{seed}-{tick}",
        source_id=f"h3h-source-{seed % 4}", modality="body_telemetry", schema_version="v1",
        ...
        provenance_chain=({"step": "d014h3h"},), privacy_classification="DERIVED_ONLY",
...
def run_case(seed: int, regime: str, horizon: int, selector=h3h_selector_callback,
             capture_trace: bool = False) -> dict[str, object]:
    scenario = SCENARIOS[regime]
    work = Path("/dev/shm")
    db = work / f"h3h-{regime}-{seed}.sqlite"
    trace_path = work / f"h3h-{regime}-{seed}.jsonl"
    ...
    events = [build_initialized_event(state["engine"].state, event_id=f"h3h:{regime}:{seed}:init",
        transaction_id=f"h3h:{regime}:{seed}:init-tx", request_id=f"h3h:{regime}:{seed}:init-req")]
    ...
    try:
        for _ in range(horizon):
            ...
            if regime == "R2" and tick == 600:
                events.append(state["engine"].commit_object_creation(partner_object(), event_id=f"h3h:{seed}:create",
                    transaction_id=f"h3h:{seed}:create-tx", request_id=f"h3h:{seed}:create-req")); state["partner_created"] = True
            ...
            if regime == "R2" and tick == 1800:
                ...
                org = load_organism(__import__('d014h3h_runtime').organism_config(db, seed, selector, trace_path, scenario))
            ...
            if regime == "R2" and tick == 2400:
                events.append(state["engine"].commit_object_visibility("social:partner:d014", occluded=True, event_id=f"h3h:{seed}:occlude",
                    transaction_id=f"h3h:{seed}:occlude-tx", request_id=f"h3h:{seed}:occlude-req")); state["partner_occluded"] = True
            if regime == "R2" and tick == 2600:
                events.append(state["engine"].commit_object_visibility("social:partner:d014", occluded=False, event_id=f"h3h:{seed}:reappear",
                    transaction_id=f"h3h:{seed}:reappear-tx", request_id=f"h3h:{seed}:reappear-req")); state["partner_reappeared"] = True
        ...
        return {"directive": DIRECTIVE, "regime": regime, "scenario": scenario, "seed": seed, "ticks": org.tick,
            ...
            "actions": actions, **extrema, "selector_call_count": sum("d014h3h_selector" in r for r in rows),
...
def proof(evidence_root: Path) -> dict[str, object]:
    ...
    if ordinary["actions"] != identity["actions"]: raise SystemExit("D014H3H_DISABLED_PARITY_FAIL")
    result = {"disabled_parity": True, "ordinary": ordinary, "identity": identity,
        "selector_contract": fingerprint(base_state()), "source": "d014h3h_selector.evaluate reused unchanged"}
    write_json(evidence_root / "D014H3H_INJECTION_AND_REPLAY_PROOF.json", result); return result
...
def main() -> None:
    ...
    if args.mode == "proof": result = proof(args.evidence_root)
        ...
        write_json(args.evidence_root / "D014H3H_REGIME_PREFLIGHT.json", result)
    elif args.mode == "freeze":
        contract = {"directive": DIRECTIVE, "baseline": BASELINE, "selector_spec": "D014H3H_SELECTOR_SPEC", "regimes": regime_spec()["regimes"],
            "scenario_map": SCENARIOS, "r0_seeds": R0_SEEDS, "known_r1": KNOWN_R1, "horizon_ticks": HORIZON,
            "holdouts": "adopt exact D014H3H_FRESH_HOLDOUT_MANIFEST; no regeneration", "production_changes": [], "formal": False}
        write_json(args.evidence_root / "D014H3H_REGIME_CONTRACT.json", contract); (args.evidence_root / "D014H3H_REGIME_CONTRACT.sha256").write_text(hashlib.sha256(canonical_bytes(contract)).hexdigest() + "\n"); result = contract
    elif args.mode == "r0":
        result = {"rows": [run_case(seed, "R0", args.horizon or HORIZON) for seed in R0_SEEDS]}
        result["all_pass"] = all(row["terminal"] == "completed" for row in result["rows"]); write_json(args.evidence_root / "D014H3H_R0_RESULTS.json", result)
    elif args.mode == "known-r1":
        result = run_case(KNOWN_R1, "R1", args.horizon or HORIZON); write_json(args.evidence_root / "D014H3H_KNOWN_R1_RESULT.json", result)
    else:
        result = {"holdouts": "adoption pending", "source_manifest": "D014H3H_FRESH_HOLDOUT_MANIFEST.json"}; write_json(args.evidence_root / "D014H3H_HOLDOUT_ADOPTION.json", result)
