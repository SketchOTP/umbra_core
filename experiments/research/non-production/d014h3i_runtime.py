"""D-014H3I no-safe composition runtime adapter and proof harness."""
...
from d014h3i_selector import evaluate
...
def selector_input(context: dict[str, Any]) -> dict[str, Any]:
    ...
    capability_effect_branches = context["effect_branches"]
    exact_effect_branches = context.get("effect_branches_exact", {})
    ...
    for index, row in enumerate(pool):
        ...
        route = _route_for(row, observations, capability_effect_branches)
        policy_context = {
            "policy_visible": True,
            "evidence_refs": [
            ...
            "proposal_class": "ACTUAL_PROPOSAL",
            "candidate_scores": dict(row.get("scores") or {}),
            "candidate_total": float(row.get("total", 0.0) or 0.0),
        }
        if route is not None:
            policy_context["route"] = route
        candidates.append({
            "candidate_ref": str(row.get("candidate_ref", f"runtime:{index}")),
    ...
    return {
        ...
        "effect_branches": capability_effect_branches,
        "effect_branches_exact": {
            str(row.get("candidate_ref", f"runtime:{index}")): list(
                exact_effect_branches.get(str(row.get("candidate_ref", f"runtime:{index}")), [])
            )
            for index, row in enumerate(context.get("candidate_pool", []))
        },
        "hard_admissibility_exact": dict(context.get("hard_admissibility_exact", {})),
...
def h3i_selector_callback(context: dict[str, Any]) -> dict[str, Any]:
    state = selector_input(context)
    result = evaluate(state)
    selected = result.get("selected")
    if isinstance(selected, dict):
        candidate = Candidate(str(selected["capability"]), dict(selected["params"]))
        no_safe_passthrough = False
    elif result.get("resolution") == "NO_SAFE_ACTION":
        current = context.get("current_candidate")
        if isinstance(current, dict) and (current.get("params") or {}).get("source") == "no_safe_action":
        ...
        no_safe_passthrough = True
    else:
        raise RuntimeError("d014h3i_invalid_selector_resolution")
    return {
        "candidate": candidate,
        "trace": {
            "selector": result,
            ...
            "candidate_score_fields": list(state["candidates"][0]["policy_context"].get("candidate_scores", {}).keys()) if state["candidates"] else [],
            "no_safe_passthrough": no_safe_passthrough,
        },
    }
...
def run_one_tick(selector: Callable | None, seed: int = 41241905, scenario: str = "S0") -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="u-h3h-", dir="/dev/shm") as tmp:
        root = Path(tmp)
    ...
    with tempfile.TemporaryDirectory(prefix="u-h3h-", dir="/dev/shm") as tmp:
        ...
        try:
            ...
            return {
                ...
                "ticks": org.tick,
                "selector_call_count": sum(1 for row in rows if "d014h3h_selector" in row),
                "post_selection_replacement_count": sum(
                    row.get("d014h3h_selector", {}).get("post_selection_replacement_count", 0)
                    for row in rows
                ),
            }
        finally:
