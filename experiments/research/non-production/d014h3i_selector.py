"""D-014H3I fresh deterministic authority-safe prospective selector with explicit no-safe composition.
...
def _branch_effects(candidate: dict[str, Any], state: dict[str, Any]) -> list[dict[str, Any]]:
    # Exact authority branches are authoritative for this exact candidate.
    # Route metadata is prospective support and must not shadow authority.
...
def _hard_inadmissible(candidate: dict[str, Any], state: dict[str, Any],
                       phys: dict[str, dict[str, float]]) -> tuple[bool, str]:
    ref = candidate.get("candidate_ref")
    exact_hard = state.get("hard_admissibility_exact", {})
    if isinstance(exact_hard, dict) and isinstance(ref, str) and ref in exact_hard:
        return (not bool(exact_hard[ref]), "runtime_authority_exact" if exact_hard[ref] else "runtime_authority_rejected")
    branches = _branch_effects(candidate, state)
    if not branches:
        return False, "authority_effects_unresolved"
    current = {name: phys[name]["value"] for name in DIMENSIONS}
    drift = state.get("drift", {})
    for branch in branches:
        effect = branch.get("effect", {})
        successor = _add_vector(current, _add_vector(effect, drift))
        if _critical(successor, phys):
            return True, "authoritative_next_decision_critical"
    return False, "authority_reachable"
...
def _annotate(candidate: dict[str, Any], index: int, state: dict[str, Any],
    ...
    hard_rejected, hard_reason = _hard_inadmissible(candidate, state, phys)
    provenance = {
    ...
    return {
        ...
        "hard_admissible": not hard_rejected,
        "hard_rejection_reason": hard_reason if hard_rejected else None,
...
def evaluate(state: dict[str, Any]) -> dict[str, Any]:
    required = {
        ...
        "effect_branches", "effect_branches_exact", "hard_admissibility_exact",
        "opportunities", "recovery_focus", "candidates",
    }
    ...
    hard_rejected = [row for row in dedup if not row["hard_admissible"]]
    eligible = [row for row in dedup if row["hard_admissible"]]
    dominance = []
    ...
    non_dominated = [row for row in eligible if row["candidate_key"] not in dominated]
    ...
    if feasible:
        ...
        # Prospective UNKNOWN is neutral, but cannot be selected as a safe
        # action. KNOWN_INFEASIBLE is evidence only and is never a fallback.
        selected = None
