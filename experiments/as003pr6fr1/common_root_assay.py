"""One-shot R6F-R1 common-root route acquisition assay.

This fresh namespace is a protocol-only repair of the accepted R6F harness
import defect. Its scientific protocol is intentionally inherited unchanged:
one ordinary organism run, no final selector, no planning reader, and no
candidate intervention. After a verified route experience is acquired, the
next decision root and its ordinary IDLE/MOVE pool are captured for offline R6E
relation analysis.
"""
...
R6FR1_BASELINE = "e5af166e86e85a5937d25b579f9256768bbd3d30"
SCENARIO = "S0"
...
def run_once() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="u-r6f-r1-", dir="/dev/shm") as temporary:
        root = Path(temporary)
        ...
        try:
            ...
            if route is None:
                return {
                    "schema": "AS003PR6FR1_OPERATIONAL_ACQUISITION_RESULT_V1",
...
def main() -> None:
    result = run_once()
    digest = _publish("AS003PR6FR1_OPERATIONAL_ACQUISITION_RESULT.json", result)
