#!/usr/bin/env python3
"""CLOSE-02V gated qualification runner.

The organism lifecycle is delegated to the existing production-native
qualification runner. This wrapper supplies the CLOSE-02V seed manifests,
...
DIRECTIVE = "UMBRA-CLOSE-02V-R"
KNOWN_R1_SEED = 57531938
EVIDENCE = Path(
    "/srv/ATLAS/100_ACTIVE/Projects/UMBRA-CORE/evidence/live-evidence/"
    "umbra-close-02v-runner-recovery-r1"
)
...
def load_seeds() -> tuple[dict[str, Any], dict[str, Any]]:
    development = json.loads((EVIDENCE / "CLOSE02VR_DEVELOPMENT_SEEDS.json").read_text())
    formal = json.loads((EVIDENCE / "CLOSE02VR_FORMAL_SEEDS.json").read_text())
...
def execute(work: Path, output: Path) -> dict[str, Any]:
    ...
    for stage, regime, seed, horizon in diagnostics:
        ...
        if result["verdict"] != "PASS":
            closeout = {
                ...
                "verdict": f"CLOSE02V_{stage}_FAIL",
            }
            durable_json(output, closeout)
            return closeout

    stages = [
        ("KNOWN_R1", "R1", [KNOWN_R1_SEED]),
        ...
        for seed in formal[regime]:
            ...
            if result["verdict"] != "PASS":
                closeout = {
                    ...
                    "rows": rows,
                    "formal_started": True,
                    "verdict": "CLOSE02V_FORMAL_INTEGRATED_VIABILITY_FAIL",
                }
                durable_json(output, closeout)
                return closeout
    ...
    closeout = {
        ...
        "rows": rows,
        "formal_started": True,
        "verdict": "CLOSE02V_VERIFIED_DENIAL_RECOVERY_REACQUISITION_QUALIFIED",
    }
    durable_json(output, closeout)
    return closeout
