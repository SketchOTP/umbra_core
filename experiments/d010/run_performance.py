...
FRAME_RING_CAP = int(THR.get("frame_ring_capacity", FRAME_RING_CAPACITY))
...
def run_100k() -> dict[str, Any]:
    ...
    try:
        ...
        bounded = max_occ <= FRAME_RING_CAP and frames_absent
...
def run_lifecycle() -> dict[str, Any]:
    ...
    out = {
        ...
        "pass": bool(continued and not failures and cycles >= (100 if not _is_smoke() else 5)),
        "smoke_scaled": _is_smoke(),
    }
    OUT.mkdir(parents=True, exist_ok=True)
...
def run_mode(mode: str) -> dict[str, Any]:
    ...
    def _tick_body() -> None:
        nonlocal rendered, exceptions, max_occ
        org.tick_once()
        max_occ = max(max_occ, len(org.frame_ring))
...
def recompose(
    ...
    temporal_over_core = _delta(p1, p0)
    ...
    inc_pass = inc_ok(temporal_over_core) and inc_ok(tkinter_over_headless) and inc_ok(tkinter_over_core)
    smoke = any(m.get("smoke_scaled") for m in (p0, p1, p2))
    out = {
        "directive": "UMBRA-D-010",
        "adaptive_soak_supplement": "S3",
        "supplement_note": (
            "D-010 Task 14 used authorized adaptive-soak Supplement S3 "
        ...
        "temporal_over_core": temporal_over_core,
...
def run_performance(*, mode: str | None = None) -> dict[str, Any]:
    ...
    for m in modes:
        payload["modes"][m] = run_mode(m)
    payload["pass"] = all(payload["modes"][m].get("pass") for m in payload["modes"])
    OUT.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    if args.mode == "100k":
        ...
        raise SystemExit(0 if r["pass"] or _is_smoke() else 1)
    ...
    for step in ("100k", "lifecycle", "P0", "P1", "P2"):
        ...
        proc = subprocess.run(
            [sys.executable, "-m", "experiments.d010.run_performance", "--mode", step],
            cwd=ROOT,
            env=os.environ.copy(),
        )
        if proc.returncode != 0 and not _is_smoke():
            raise SystemExit(f"step_failed:{step}:{proc.returncode}")
        results[step] = "ok"
    summary = recompose()
    print(json.dumps({"steps": results, "pass": summary["pass"]}, indent=2))
    raise SystemExit(0 if summary["pass"] or _is_smoke() else 1)
