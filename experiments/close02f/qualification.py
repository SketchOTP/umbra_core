...
EVIDENCE = Path('/srv/ATLAS/100_ACTIVE/Projects/UMBRA-CORE/evidence/live-evidence/umbra-close-02f-final-authority-qualification-r1')
    ...
    try:
        for _ in range(horizon):
            ...
            if regime == 'R3' and tick == 3600:
                organism.embodiment_adapter.swap_profile(MINIMAL_CREATURE_BODY.profile_id, origin='CLOSE02F_R3_PREREGISTERED')
...
def seeds():
    development = json.loads((EVIDENCE / 'CLOSE02F_DEVELOPMENT_SEEDS.json').read_text())['seeds']
    formal = json.loads((EVIDENCE / 'CLOSE02F_FORMAL_SEEDS.json').read_text())['seeds']
...
def execute(work: Path, output: Path) -> dict[str, Any]:
    ...
    for stage, regime, population in stages:
        for seed in population:
            ...
            if row['terminal'] != 'completed':
                result = {'directive': DIRECTIVE, 'phase': 'development', 'terminal_stage': stage, 'rows': rows, 'formal_started': False, 'verdict': f'CLOSE02F_{stage}_FAIL'}
    ...
    for regime in ('R0', 'R1', 'R2', 'R3'):
        for seed in formal[regime]:
            ...
            rows.append(row)
            if row['terminal'] != 'completed':
                result = {'directive': DIRECTIVE, 'phase': 'formal', 'terminal_stage': row['stage'], 'rows': rows, 'formal_started': True, 'verdict': 'CLOSE02F_FORMAL_INTEGRATED_VIABILITY_FAIL'}
                durable_json(output, result)
                return result
    result = {'directive': DIRECTIVE, 'phase': 'formal', 'terminal_stage': 'complete', 'rows': rows, 'formal_started': True, 'verdict': 'CLOSE02F_FINAL_AUTHORITY_INTEGRATED_VIABILITY_QUALIFIED'}
    durable_json(output, result)
    return result
